...
class NRRDViewer {
        ...
        reader.onload = (e) => {
            try {
                ...
                if (fileName.endsWith('.nii') || fileName.endsWith('.nii.gz')) {
                    ...
                    throw new Error('Unsupported file format. Please use .nrrd, .nii, or .nii.gz files.');
                }
    ...
    renderSlice() {
        ...
        for (let i = 0; i < sliceSize; i++) {
            ...
            if (value > 0) {
                ...
                const color = this.categoryColors[colorIndex % this.categoryColors.length];
    ...
    initThreeJS() {
        ...
        console.log('Scene created with background:', this.scene.background.getHex());
    ...
    createSTLMesh() {
        ...
        console.log('Creating mesh with vertices:', this.stlData.vertices.length / 3, 'triangles:', this.stlData.triangleCount);
        ...
        console.log('Normals array length:', this.stlData.normals ? this.stlData.normals.length : 'none');
        console.log('Indices array length:', this.stlData.indices ? this.stlData.indices.length : 'none');
        ...
        if (!(this.stlData.vertices instanceof Float32Array)) {
            console.error('Vertices is not Float32Array:', typeof this.stlData.vertices);
        ...
        if (this.stlData.vertices.length % 3 !== 0) {
            console.error('Vertices array length not divisible by 3:', this.stlData.vertices.length);
        ...
        for (let i = 0; i < Math.min(9, this.stlData.vertices.length); i++) {
            if (!isFinite(this.stlData.vertices[i])) {
                console.error('Invalid vertex data at index', i, ':', this.stlData.vertices[i]);
        ...
        try {
            geometry.setAttribute('position', new THREE.BufferAttribute(this.stlData.vertices, 3));
            console.log('Position attribute set, vertex count:', this.stlData.vertices.length / 3);
        } catch (error) {
        ...
        console.log('Using non-indexed geometry to avoid Three.js compatibility issues');
        ...
        document.addEventListener('keydown', (event) => {
            ...
            switch(event.key.toLowerCase()) {
                ...
                case 'f':
                    ...
                    this.scene.fog = this.scene.fog ? null : new THREE.Fog(0xffffff, 10, 100);
